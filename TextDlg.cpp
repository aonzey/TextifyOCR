...
BOOL CTextDlg::OnInitDialog(CWindow wndFocus, LPARAM lInitParam)
{
	...
	if(m_hasOcrText)
	{
		// OCR mode: use provided text and position
		ptEvent = m_ocrPoint;
		strText = m_ocrText;
		rcAccObject = CRect{ ptEvent, CSize{ 200, 20 } };
	}
	else
	{
		ptEvent = *reinterpret_cast<CPoint*>(lInitParam);
		...
		if(wndAcc)
		{
			...
			if(wndAccRoot)
			{
				...
				if(::GetClassName(wndAccRoot, szBuffer, ARRAYSIZE(szBuffer)) &&
					wcscmp(szBuffer, L"TextifyOCREditDlg") == 0)
				{
