...
bool UserConfig::LoadFromIniFile()
{
	...
	// OCR hotkey: default is Ctrl+Shift+O (key=0x4F, ctrl=1, shift=1)
	m_ocrHotKey.key = GetPrivateProfileInt(L"ocr", L"key", 0x4F, iniFilePath);
	m_ocrHotKey.ctrl = GetPrivateProfileInt(L"ocr", L"ctrl", 1, iniFilePath);
	m_ocrHotKey.alt = GetPrivateProfileInt(L"ocr", L"alt", 0, iniFilePath);
	m_ocrHotKey.shift = GetPrivateProfileInt(L"ocr", L"shift", 1, iniFilePath);

	// OCR Python path: optional, falls back to auto-detection if empty
...
bool UserConfig::SaveToIniFile() const
{
	...
	// Save OCR hotkey
	str.Format(L"%d", m_ocrHotKey.key);
	if(!WritePrivateProfileString(L"ocr", L"key", str, iniFilePath))
		succeeded = false;

	str.Format(L"%d", m_ocrHotKey.ctrl ? 1 : 0);
	if(!WritePrivateProfileString(L"ocr", L"ctrl", str, iniFilePath))
		succeeded = false;

	str.Format(L"%d", m_ocrHotKey.alt ? 1 : 0);
	if(!WritePrivateProfileString(L"ocr", L"alt", str, iniFilePath))
		succeeded = false;
