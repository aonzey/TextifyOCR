...
namespace
{
	int RunApp(HINSTANCE hInstance)
	{
		CHandle hMutex(::CreateMutex(NULL, TRUE, L"textifyocr_app"));
		if(hMutex && GetLastError() == ERROR_ALREADY_EXISTS)
		{
			CWindow wndRunning(::FindWindow(L"TextifyOCR", NULL));
			if(wndRunning)
			{
		...
		RegisterDialogClass(L"TextifyOCR", hInstance);
		RegisterDialogClass(L"TextifyOCREditDlg", hInstance);
		...
		UnregisterClass(L"TextifyOCR", hInstance);
		UnregisterClass(L"TextifyOCREditDlg", hInstance);
